...
## Streaming Pipeline (Real-Time Demo)

While the batch pipeline handles the deep, history-dependent predictive logic, the streaming pipeline demonstrates a separate, complementary capability: **immediate, windowed aggregation as events arrive**, rather than waiting for a batch job to run.

**How it works:**
1. A local Python script (`scripts/send_sensor_events.py`) simulates real-time sensor data by sending individual readings from the source CSV into Azure Event Hub, one at a time with a short delay — mimicking how a real sensor would report continuously rather than in a daily batch file.
2. Azure Stream Analytics listens to the Event Hub continuously and applies a **tumbling window** aggregation — a windowing concept specific to stream processing (no equivalent in standard batch SQL), since a stream has no fixed end to aggregate "the whole dataset" the way batch can:

```sql
SELECT
    machine_id,
    System.Timestamp() AS window_end,
    AVG(temperature_c) AS avg_temp_last_minute,
    AVG(vibration_mm_s) AS avg_vibration_last_minute,
    MAX(temperature_c) AS max_temp_last_minute
INTO [events]
FROM [sensor-events]
GROUP BY machine_id, TumblingWindow(second, 30)
```

3. Results write continuously to the Data Lake as each 30-second window closes — not a single final output, but an ongoing stream of small result batches for as long as the job runs.

**Scope note, stated honestly:** this was run as a short, deliberate demo burst (roughly 5–8 minutes total, Event Hub → Stream Analytics → Output), not a sustained production monitor — Stream Analytics bills per hour regardless of data volume, so it was started, used briefly to demonstrate and verify the mechanism, and stopped immediately. A production deployment would run continuously against real sensor hardware.

**Why the baseline-comparison logic stays in batch, not streaming:** the health-flagging logic depends on each machine's full 15-day historical baseline — information a true, unbounded stream doesn't have access to in the same way a complete batch dataset does. Streaming and batch are used here for what each is actually suited to, not as a redundant reimplementation of the same logic twice.
